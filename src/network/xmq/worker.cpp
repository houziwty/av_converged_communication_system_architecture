...
#include "network/xmq/worker.h"
using namespace framework::network::xmq;

Worker::Worker() : thread{nullptr}, stopped{ false }
{}
	...
	if (Error_Code_Success == ret)
	{
		WorkerPtr ptr{
			boost::make_shared<Worker>(
				boost::bind(&Worker::afterWorkerPolledDataHandler, this, _1))};
...
{	
	...
	if(Error_Code_Success == ret)
	{
		ThreadPool().get_mutable_instance().createNew(
			boost::bind(&Worker::keepAliveWorkerThread, this, appid, xmqid, interval));
	}

	return ret;
}

void Worker::pollDataFromWorkerThread()
{
...
void Worker::keepAliveWorkerThread(
	const std::string appid, 
	const std::string xmqid,  
	const int interval/* = 30*/)
{
