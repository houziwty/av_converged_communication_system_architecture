...
static void http_session_onsend(void* param, int code, size_t bytes)
{
	...
	if (0 == r && 0 == code && NULL == session->wsupgrade /* exclude websocket mode */)
	{
		...
		if (session->remain > 0)
			...
			r = 0;//aio_transport_recv(session->transport, session->data, HTTP_RECV_BUFFER);
	}
...
int http_server_send_vec(struct http_session_t *session, const struct http_vec_t* vec, int num, http_server_onsend onsend, void* param)
{
	...
	return 0;//aio_transport_send_v(session->transport, session->vec.vec, session->vec.count);
}
...
{
	return http_session_release(session);
...
{
	session->data = data;
	session->remain += bytes;
	http_session_onrecv(session, code, bytes);
	return 0;
}
